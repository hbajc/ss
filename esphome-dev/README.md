...
这个附加组件允许您编写配置，将您的微控制器直接转变为智能家居设备，通过 Home Assistant **无需编程经验**。
您所需做的就是编写 YAML 配置文件；其余的（空中更新、编译）都由 ESPHome 处理。
...
使用 ESPHome，您可以通过几行 YAML 直接生成定制的固件。例如，要包括一个 [DHT22][dht22] 温湿度传感器，您只需在配置文件中包含 8 行 YAML：
