...
这个插件允许你直接通过 Home Assistant 编写配置并将微控制器变成智能家居设备，**无需编程经验**。
你只需要编写 YAML 配置文件；其他的（空中更新、编译）都由 ESPHome 处理。
...
使用 ESPHome，你可以从几行 YAML 直接生成定制的固件。例如，要包含一个 [DHT22][dht22] 温湿度传感器，你只需要在配置文件中包含 8 行 YAML：
