...
这个附加组件允许你编写配置，并将你的微控制器直接转换为智能家居设备，通过 Home Assistant **无需编程经验**。你只需编写 YAML 配置文件；其余的（空中更新、编译）由 ESPHome 处理。
...
使用 ESPHome，你可以从几行 YAML 直接生成定制的固件。例如，要包含一个 [DHT22][dht22] 温湿度传感器，你只需在你的配置文件中包含 8 行 YAML：
