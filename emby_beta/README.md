...
_感谢所有为我的仓库加星的人！要加星，请点击下面的图片，然后它就会在右上方。谢谢！_
...
[emby](https://emby.media/) 组织视频、音乐、直播电视和个人媒体库中的照片，并将它们流式传输到智能电视、流媒体盒和移动设备。这个容器被打包为一个独立的 emby 媒体服务器。

这个插件基于来自 linuxserver.io 的 [docker 镜像](https://github.com/linuxserver/docker-emby)。
最初的插件版本 : https://github.com/petersendev/hassio-addons

## 配置

Webui 可以在 `<你的-IP>:8096` 找到，或者通过 Ingress 在 Home Assistant 中找到。

```yaml
PGID: user
GPID: user
TZ: timezone
localdisks: sda1 #放入硬盘的硬件名称以逗号分隔，或其标签。例如 sda1, sdb1, MYNAS...
