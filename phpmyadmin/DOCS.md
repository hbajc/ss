...
此插件专门用于管理官方的 Home Assistant MariaDB 插件。
...
**注意**: _这只是一个示例，请不要直接复制粘贴！创建您自己的配置！_
...
`log_level` 选项控制插件的日志输出级别，可以设置为更加详细或简洁，这在处理未知问题时可能会很有用。可能的值包括：
...
发布基于 [语义版本控制][semver]，使用 `MAJOR.MINOR.PATCH` 格式。简而言之，版本将根据以下规则递增：
