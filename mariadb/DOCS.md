...
该部分定义在 MariaDB 中创建用户的定义。 [创建用户][createuser] 文档。
...
该部分授予 MariaDB 中用户的特权。 [授予][grant] 文档。
...
要授予该用户的特权列表，来自 [授予][grant]，如 `SELECT` 和 `CREATE`。
如果省略，则授予用户 `ALL PRIVILEGES`。限制 Home Assistant 使用的用户的特权
不推荐，但如果您想允许其他应用程序查看记录器数据，应创建一个有限的
只读访问数据库的用户。

### 选项：`mariadb_server_args`（可选）

一些用户在大数据库上进行 Home Assistant 架构更新时遇到 [错误][migration-issues]。
