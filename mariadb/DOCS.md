...
授予该用户的权限列表，来自 [授权][grant]，如 `SELECT` 和 `CREATE`。
如果省略，授予用户 `ALL PRIVILEGES`。不建议限制 Home Assistant 使用的用户的权限，但如果您想允许其他应用程序查看记录者数据，应该创建一个仅限只读访问数据库的用户。

### 选项：`mariadb_server_args`（可选）

一些用户在大数据库上进行 Home Assistant 架构更新时遇到过 [错误][migration-issues]。
